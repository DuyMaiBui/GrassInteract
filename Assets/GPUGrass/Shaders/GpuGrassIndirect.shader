...
Shader "GPUGrass/IndirectGrass"
{
    Properties
    {
        ...
        // Shadow properties (all default OFF/0 — keyword-off path is the plain unlit-stylized baseline).
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Shadow feature toggles (shader_feature_local, all default OFF).
            // _RECEIVE_SHADOWS: sample the main-light shadow on the unlit-stylized path → grass darkens in shadow.
            // _SHADOW_TINT:     modulate the received shadow term by _ShadowStrength/_ShadowTint.
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                // Shadow uniforms (only read under their respective keywords; defaults are inert).
            ...
            struct Varyings
            {
                ...
                // Only the receive-shadow path consumes a world-space position (for the shadow coord).
                // On the plain unlit path this interpolator is stripped — no per-fragment bandwidth cost.
            ...
            Varyings vert(float4 posOS : POSITION, float2 uv : TEXCOORD0, uint instanceID : SV_InstanceID)
            {
                ...
                // Only the receive-shadow path needs the world-space position (shadow coord in frag).
                // On the plain unlit path this write is compiled out — no interpolator bandwidth.
                #if defined(_RECEIVE_SHADOWS)
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                // ── Unlit-stylized path ───────────────────────────────────────
                // Base map × root→tip color gradient. When _RECEIVE_SHADOWS is ON, sample the
                // main-light shadow and multiply (or tint) the output so grass darkens in shadow —
                // no full PBR lighting required.
