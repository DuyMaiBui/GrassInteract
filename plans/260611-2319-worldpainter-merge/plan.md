...
**.meta/GUID safety:** every file move must move the asset AND its `.meta` as a pair (`git mv X.cs new/ && git mv X.cs.meta new/`) — `git mv X.cs` alone does NOT carry `X.cs.meta`. Move ALL pairs BEFORE any `refresh_unity`; never let Unity refresh while a `.cs` exists without its `.meta` (it regenerates a NEW GUID and breaks every asset/scene/prefab reference). NEVER plain delete+recreate. Verify GUIDs preserved (new meta `guid:` == `git show HEAD:old.meta` `guid:`) before refreshing. **This is the R2 failure that killed the first P3 attempt (2026-06-11) — metas were not paired, Unity regenerated GUIDs.**
...
| R2 | **`.meta`/GUID loss on file move breaks scene/prefab/asset references** | 4 | 5 | **20** | Move `.cs` AND `.cs.meta` as a PAIR (`git mv` both); `git mv X.cs` alone does NOT move the meta. Move ALL pairs BEFORE any `refresh_unity` — a refresh with an orphaned `.cs` regenerates a new GUID. Verify new-meta `guid:` == HEAD old-meta `guid:` BEFORE refreshing. **MATERIALIZED in P3 attempt #1 (metas not paired → Unity regenerated GUIDs); reset to baseline, instruction corrected.** | P3,P4,P5 |
