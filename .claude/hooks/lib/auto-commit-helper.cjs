 ...
 *
 * Scope safety: staging is path-scoped (`git add .claude/`) and the commit
 * names the staged `.claude/` files explicitly after `--`, so the commit can
 * only ever contain kit-sync `.claude/` paths — unrelated non-`.claude/` work
