...
const T1K = {
  ...
  // diverging across subagent processes. See lesson-collector.cjs §teammate-capture
 ...
 * from the harness, breaking parent↔teammate correlation. Project root is
...
function drainTeammateQueues(sessionKey) {
  ...
  } catch { /* tmpdir unreadable — return empty */ }
  return { markers, files };
}

module.exports = { T1K, SENSITIVE_PATTERNS, SAFE_PATTERNS, SECRET_TOKEN_PATTERNS, parseHookStdin, getModuleNames, getModuleEntries, isTelemetryEnabled, ensureTelemetryDir, findProjectRoot, dedupGuard, warnOnce, checkGhOrgScope, resolveClaudeDir, resolveProjectDir, isT1KMetadata, deriveProjectName, getHomeDir, walkUpForClaudeDir, readTelemetryContext, readTelemetryEndpoint, getGhToken, countErrorsSince, readActivatedSkillsSince, todayDateStr, readFeatureFlag, isTeammateContext, computeTeammateSessionKey, teammateQueuePath, drainTeammateQueues };
