...
### D.0 — `context: fork` decision rule (CRITICAL, empirically verified 2026-05-24)

**`context: fork` STRIPS `Agent` (and the legacy `Task` alias) from the skill body's tool scope.** `tools:` frontmatter array CANNOT add `Agent` back, because `Agent` is NOT in the deferred-tools registry — only `TaskCreate`/`TeamCreate`/`AskUserQuestion`/etc. are lazy-loadable into forks.

| Skill behavior | Use `context: fork`? |
|---|---|
| Body calls `Agent(...)` / "Use the Task tool" / spawns sub-agents | **NO — must run in main session** |
| Body calls `TeamCreate(...)` directly | NO — TeamCreate needs Agent gate as a precondition (see t1k-team Step 1) |
| Body only reads files, runs analysis, writes plan/research docs (no spawn) | YES — saves tokens via prompt-cache reuse |
| Body is documentation-only / reference (no logic) | N/A (typically no `context:` declared) |

**CI gate:** `validate-skill-context-fork-spawn-usage.cjs` (theonekit-release-action) fails the build if any SKILL.md has `context: fork` AND its body contains `Agent(`, `Task(`, `TeamCreate(`, or "Use the Agent tool" / "Use the Task tool". Mechanically enforced — no need to memorize.

### D.1 — Other sub-agent spawning rules
...
## K. Kit-shipped SKILL.md body tightness (multiplier-across-consumers cost)

Every line in a kit-shipped SKILL.md loads into every consumer's session context. Cost is multiplicative: `lines × ~50 consumers × N sessions/day`. Incident anecdotes, dated postmortems, and PR refs are valuable as history — but in `references/`, where they load only on demand.

| Red flag | Severity | Fix |
|---|---|---|
| `Originating incident: <date> ...` paragraph in body | High | Move to `references/incidents.md`; body cites "Related: <issue#>" |
| `Verified failure mode <date> session ...` prose | High | Move to a reference; one-line link from body if needed |
| `Recorded as a fix <date> after ...` postmortem in body | High | Reference + one-line "Why" in body |
| Specific PR numbers cited inline (`PR #277 added X`) | Medium | Either trim to "see #277" footer, OR delete entirely (git log answers) |
| Commit hashes in backticks (`` `a3398b69` ``) | Medium | Drop — git blame answers. Hashes drift after rebase/squash anyway |
| `v1.X.Y regression bug` / `Verified failure 2026-MM-DD` anecdotes | Medium | Move to reference if it matters; usually drop |
| Multi-line bail-message text quoted verbatim in body | High | One-line summary in body; full text in `references/<feature>-bail.md` |
| "Real-world miss (date, session ID)" paragraph | High | Move to reference |

**One-line refs ARE fine** — `Issues addressed: #163, #208, #259` is exempt. Readers click through. Only multi-line incident prose and inline anecdotes count as bloat.

**CI gate:** `validate-skill-body-tightness.cjs` (theonekit-release-action) counts incident-markers per body. WARN at >2, ERROR at >5. WARN-only grace window for ecosystem migration; STRICT after.

**Test:** count lines added to a kit-shipped SKILL.md in your PR. If >20 lines for one fix, most of it probably belongs in `references/`.
