...
## Cross-Demo Balance Table Workflow

When auditing multiple demos in a single session, maintain a cross-demo balance table to catch systematic drift before it compounds:

1. Open `docs/wiki/` for each demo and record: demo name, entity count, avg TTK (time-to-kill), win-rate skew, any outstanding balance bug.
2. Sort by TTK — outliers (2× the median) indicate formula drift (missing stat modifier, wrong formula constant).
3. For each flagged demo, run `t1k:balance` with the demo name to generate the per-demo DPS/EHP report.
4. Fix the largest TTK outlier first — it usually shares the root cause (e.g., `DerivedCombatStats` not including a new stat) with the others.
5. After fixes, re-run cross-demo table and confirm all TTKs within 1.5× of median.

Source: review-260520-round3-production.md §R3B
