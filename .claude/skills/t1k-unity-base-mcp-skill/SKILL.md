...
- **New-file `CS0103 'X' does not exist` / phantom `CS0246` on a SIBLING type after `scope=scripts` compile.** `refresh_unity(mode="force", scope="scripts")` recompiles EXISTING assemblies but does NOT import a brand-new `.cs` file into the AssetDatabase (the file has no `.meta` yet). Symptoms: (a) `CS0103 'X' does not exist`; (b) the more confusing **phantom `CS0246` "type or namespace not found" on a same-namespace SIBLING type** that references the new file — the sibling looks broken but the real cause is the new file was never imported. **Fix:** after writing ANY brand-new `.cs` file to disk, call `refresh_unity(mode="force", scope="all")` ONCE so the AssetDatabase imports the new asset, THEN recompile (`scope="scripts"` alone is sufficient ONLY for edits to already-tracked files). `execute_menu_item("Assets/Refresh")` is an equivalent one-time import trigger. `manage_asset(action="reimport", path="Assets/Foo.cs")` does NOT work for untracked files (see existing gotcha below).
