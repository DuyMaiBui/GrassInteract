...
| `references/editor-availability-precheck.md` | MANDATORY pre-check before MCP compile/verify/test — lock file + pgrep, decision table, spawn-brief requirement |

## Gotchas

- **Editor-bound sub-agents MUST verify the Unity Editor is alive BEFORE any MCP compile/verify/test call.** Check `ls Temp/UnityLockfile` and `pgrep -af "Unity.*<ProjectName>"` as the first step — both absent means the editor is down. A sub-agent that skips this can burn its entire token budget on repeated `No Unity Editor instances` / timeout errors with zero file writes. If the editor is down: do filesystem-only work (`Write`, `Edit`, `Bash`) for the phase and report "Editor was down, MCP skipped." Full decision table and spawn-brief requirement: `references/editor-availability-precheck.md`.
...
- **Adding a URP `ScriptableRendererFeature` — `manage_render_pipeline` can't do it; YAML-edit the `*_Renderer.asset`.** `manage_render_pipeline` cannot ADD a renderer feature (and is unregistered on some bridge builds). A renderer feature is a sub-asset `MonoBehaviour` inside the `<Name>_Renderer.asset`. Add it by editing that `.asset` YAML directly — append a sub-asset block carrying the feature script's GUID (from its `.cs.meta`), its material object-ref (`{fileID:..., guid:..., type:2}`), and the `RenderPassEvent` as an int — then `manage_asset(action="reimport", path=...)`. URP rebuilds `m_RendererFeatureMap` on reimport, wiring the feature in. Companion gotchas in the same URP-effect flow: (1) `manage_scene(action="save")` **misreports the saved scene name** — verify the real save via `get_loaded_scenes` `isDirty:false` + the `.unity` file mtime, not the returned name; (2) `manage_gameobject(action="create", component_properties=...)` does **not** bind ScriptableObject / Material object-refs — set those afterward with `manage_components(action="set_property", value={guid:...})`; (3) a global shader var set via `Shader.SetGlobalFloat` must be declared **OUTSIDE** the `UnityPerMaterial` CBUFFER or the SRP batcher breaks (per-material CBUFFER members must be per-material, not global).
- **URP 17.3 fullscreen / blit shaders must include `Blit.hlsl` from the CORE package, not the universal package.** Use `#include "Packages/com.unity.render-pipelines.core/Runtime/Utilities/Blit.hlsl"` — it provides `Vert` / `Varyings` / `_BlitTexture` / `sampler_LinearClamp`. Including the universal-package ShaderLibrary path (`Packages/com.unity.render-pipelines.universal/ShaderLibrary/Blit.hlsl`) errors with `Couldn't open include file` on URP 17.3. Mirror URP's own `Shaders/Utils/Blit.shader` include line, and verify the path against the installed package version (the core-package location is stable across recent URP).
- **`manage_components(action="set_property")` reports success but CANNOT bind a scene-object reference inside an array field.** Single asset refs bind fine via `{guid:...}`, but `Transform[]` / scene-object-array elements stay `null` whether you pass the GameObject's or the Transform component's instance ID — `set_property` returns ok and silently no-ops the array binding. Fix: `manage_scene(action="save")` the scene, then edit the array element `fileID`s directly in the scene YAML (map each child Transform's `fileID` from the parent's `m_Children` list, disambiguating by `m_LocalPosition`), then `manage_scene(action="load")` to reload from disk so the bound refs take effect.
- **Entering Play Mode directly in a gameplay scene throws a FALSE `InvalidDataException 'Blueprint doesn't contain Id X'`.** GameFoundation blueprint loading runs in the boot/loading scene (the `DataLoader` is an `IAsyncEarlyLoadable` that executes during app init there). Pressing Play straight into a gameplay scene skips that init, so currency/item blueprints are empty and `GetCurrencyValue` (and any blueprint lookup) throws `InvalidDataException: Blueprint doesn't contain Id <X>` — NOT a real bug, just missing init. For any smoke test touching currency / inventory / blueprints, enter Play Mode from the **boot/loading scene** so app init runs and blueprints load before the gameplay scene activates.
...
- **New-file `CS0103 'X' does not exist` / phantom `CS0246` on a SIBLING type after `scope=scripts` compile — `scope=scripts` skips the disk scan.** A brand-new `.cs` file added on disk is NOT imported by `refresh_unity(mode="force", scope="scripts", compile="request")`: that call returns `refresh_triggered: false` and recompiles EXISTING assemblies WITHOUT the new file (it has no `.meta` yet — the AssetDatabase never scanned disk). Symptoms: (a) `CS0103 'X' does not exist`; (b) the more confusing **phantom `CS0246` "type or namespace not found" on a same-namespace SIBLING type** that references the new file — the sibling looks broken but the real cause is the new file was never imported. **Fix:** after writing ANY brand-new `.cs` file to disk, call `refresh_unity(mode="force", scope="all", compile="request")` ONCE (or `execute_menu_item("Assets/Refresh")`) so the AssetDatabase imports the new asset, THEN the type resolves on the next compile (`scope="scripts"` alone is sufficient ONLY for edits to already-tracked files). `manage_asset(action="reimport", path="Assets/Foo.cs")` does NOT work for untracked files (see existing gotcha below). Mnemonic: `scope=scripts` recompiles what Unity already tracks; only `scope=all` discovers what's new on disk.
