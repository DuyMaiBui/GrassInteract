...
These are thought patterns that masquerade as productive but are actually symptoms of model over-confidence. If you catch yourself drafting one, STOP and return to Scout or Diagnose.
...
| "Just try changing X" | Random fixes waste tokens and create new bugs. Diagnose first. |
| "It's probably X" | "Probably" = guessing. Use structured diagnosis. Verify first. |
| "One more fix attempt" (after 2+) | 3+ failures = wrong approach. Question architecture; talk to user. |
