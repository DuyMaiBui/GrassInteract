...
function output(data) {
  if (jsonOutput) {
    ...
    if (data.success) {
      ...
      console.log(`   Worktree mode: ${data.worktreeMode}`);
      if (data.worktreeMode === 'secondary') {
        if (data.mainWorktreePath) console.log(`   Main worktree: ${data.mainWorktreePath}`);
        console.log(`   This worktree: ${data.thisWorktreePath}`);
      }
...
function cmdInfo() {
  ...
  // Detect whether this location is a secondary worktree (`.git` is a file, not a dir)
  // and surface the main worktree path so users can tell where destructive ops will land.
  const gitDotPath = path.join(gitRoot, '.git');
  let worktreeMode = 'main';
  let mainWorktreePath = null;
  let thisWorktreePath = gitRoot;
  try {
    if (fs.statSync(gitDotPath).isFile()) {
      worktreeMode = 'secondary';
      const gitContent = fs.readFileSync(gitDotPath, 'utf8');
      const match = gitContent.match(/^gitdir:\s*(.+?)\/\.git\/worktrees\//);
      if (match) mainWorktreePath = match[1];
    }
  } catch { /* .git missing → not a repo path; checkGitRepo would have caught it */ }
