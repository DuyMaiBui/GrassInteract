...
# OpenAI-compatible backend (LiteLLM proxy / model-router CCS).
# Active only when GEMINI_OPENAI_BASE_URL is set; needs no google-genai SDK.
try:
    import openai_compat as oc
    from openai_compat import find_openai_base_url
    OPENAI_COMPAT_AVAILABLE = True
except ImportError:
    OPENAI_COMPAT_AVAILABLE = False
    oc = None  # type: ignore[assignment]
    find_openai_base_url = None  # type: ignore[assignment]
...
def find_api_key() -> Optional[str]:
    ...
    2. PROJECT/.claude/skills/t1k-extended-multimodal/.env (skill-specific)
    3. PROJECT/.claude/skills/.env (shared skills)
    4. PROJECT/.claude/.env (project global)
    5. ~/.claude/skills/t1k-extended-multimodal/.env (user skill-specific)
    ...
    if CENTRALIZED_RESOLVER_AVAILABLE:
        ...
        return resolve_env('GEMINI_API_KEY', skill='t1k-extended-multimodal')
...
def batch_process_openai_compat(
    base_url: str,
    files: List[str],
    prompt: str,
    model: str,
    ...
    """Route analyze (vision) + generate (image) through an OpenAI-compatible
    endpoint. Returns result dicts in the same shape print_results expects."""
    api_key = find_api_key()
    if not api_key:
        print("Error: GEMINI_API_KEY not found (used as the bearer token for the "
              "OpenAI-compatible endpoint set via GEMINI_OPENAI_BASE_URL).")
        sys.exit(1)

    if verbose:
        print(f"✓ OpenAI-compatible backend: {base_url} (model={model}, task={task})", file=sys.stderr)

    if task not in oc.SUPPORTED_TASKS:
        return [{
            'file': (files[0] if files else 'generated'),
            'status': 'error',
            'error': (f"Task '{task}' is not supported via an OpenAI-compatible backend "
                      f"(GEMINI_OPENAI_BASE_URL is set). Supported: {sorted(oc.SUPPORTED_TASKS)}. "
                      f"Unset GEMINI_OPENAI_BASE_URL to use native Gemini for this task."),
        }]

    if dry_run:
        print("DRY RUN MODE (OpenAI-compatible) - No API calls will be made")
    ...
    if task == 'generate':
        try:
            img_bytes, ext, text = oc.generate_image(base_url, api_key, model, prompt)
            out_dir = Path(output_file).parent if output_file else Path('.')
            out_dir.mkdir(parents=True, exist_ok=True)
            out_path = out_dir / f"generated_{int(time.time())}.{ext}"
            out_path.write_bytes(img_bytes)
            if verbose:
                print(f"  Saved image to: {out_path}")
            result: Dict[str, Any] = {'status': 'success', 'generated_images': [str(out_path)], 'model': model}
            if text:
                result['response'] = text
            results.append(result)
        except Exception as e:  # noqa: BLE001 - surface any backend failure per-item
            results.append({'file': 'generated', 'status': 'error', 'error': str(e)})
        return results

    # task == 'analyze' (vision) — image inputs only over this backend
    for file_path in files:
        if not oc.is_image(file_path):
            results.append({
                'file': str(file_path),
                'status': 'error',
                'error': ("OpenAI-compatible backend supports image inputs only for 'analyze'. "
                          f"'{file_path}' is not an image — use native Gemini (unset GEMINI_OPENAI_BASE_URL) "
                          "for audio/video/PDF."),
            })
            continue
        try:
            text = oc.analyze_image(base_url, api_key, model, prompt, str(file_path))
            results.append({'file': str(file_path), 'status': 'success', 'response': text})
...
def batch_process(
    ...
    # OpenAI-compatible backend short-circuit (LiteLLM proxy / model-router CCS).
    oc_base = find_openai_base_url() if OPENAI_COMPAT_AVAILABLE else None
    if oc_base:
        return batch_process_openai_compat(
            oc_base, files, prompt, model, task, output_file, verbose, dry_run
        )
    ...
    if not api_key:
        ...
        print("2. Show hierarchy: python ~/.claude/scripts/resolve_env.py --show-hierarchy --skill t1k-extended-multimodal")
        print("3. Quick setup: export GEMINI_API_KEY='your-key'")
        print("4. Create .env: cd ~/.claude/skills/t1k-extended-multimodal && cp .env.example .env")
